...
mazeFromArray = fmap (Node False)

adjacents :: Maze -> Point -> [Point]
adjacents mz =
  let offsets (xi, yi) = [(x + xi, y + yi) | (x, y) <- [(1, 0), (-1, 0), (0, 1), (0, -1)]]
   in filter (inRange $ bounds mz) . offsets

setVisited :: [Point] -> Maze -> Maze
setVisited ps mz =
  let setVisitedOne (Node _ n) = Node True n
   in mz // zip ps (map (setVisitedOne . (mz !)) ps)

dijkstra :: Point -> Maze -> [(Point, Int)] -> [(Point, Int)]
dijkstra endP mz (pqh : pqrest)
  | endP == fst pqh = [pqh]
  | isVisited . (mz !) . fst $ pqh = dijkstra endP mz pqrest
  | all (isVisited . (mz !)) . adjacents mz . fst $ pqh = dijkstra endP mz pqrest
  | otherwise =
    let mz' = setVisited [fst pqh] mz
        adjs = adjacents mz . fst $ pqh
        fltadjs = filter (not . all (isVisited . (mz' !)) . adjacents mz) . filter (not . isVisited . (mz' !)) $ adjs
        pqadjs = map (id &&& (+ snd pqh) . getCost . (mz !)) fltadjs
        pq' = sortOn snd $ pqadjs ++ pqrest
     in pqh : dijkstra endP mz' pq'

part1 :: Array (Int, Int) Int -> Int
part1 arr = snd . last . dijkstra (snd . bounds . mazeFromArray $ arr) (mazeFromArray arr) $ [((0, 0), 0)]
